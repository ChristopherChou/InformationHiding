...
public:
	...
	int inset(double* picture, double* picture_inset, int *logo, double strength);//完成水印的嵌入
	int inset8x8(double pic[8][8], double strength, int b);//对8x8的块进行嵌入
	...
	int transfer(int* logo_decode, unsigned char* logo_buf);//把logo图转为位的形式
};
